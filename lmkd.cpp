...
static const bool isBpfRingBufferSupported = android::bpf::isAtLeastKernelVersion(5, 8, 0);
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    } else if (!isBpfRingBufferSupported && (vs.field.pgscan_direct != init_pgscan_direct)) {
...
static void direct_reclaim_state_change(int data __unused, uint32_t events __unused,
     ...
     * state.
     */
    for (int i = mem_events.size() - 1; i >= 0; i--) {
        if (mem_events[i].type == MEM_EVENT_DIRECT_RECLAIM_BEGIN) {
            direct_reclaim_start_tm = curr_tm;
            return;
        }
        if (mem_events[i].type == MEM_EVENT_DIRECT_RECLAIM_END) {
            direct_reclaim_start_tm.tv_sec = 0;
            direct_reclaim_start_tm.tv_nsec = 0;
            return;
        }
...
static bool init_monitors() {
    ...
    if (isBpfRingBufferSupported && !init_direct_reclaim_monitoring()) {
...
static bool update_props() {
    ...
    if (!isBpfRingBufferSupported && direct_reclaim_threshold_ms > 0) {
        ALOGW("direct_reclaim_threshold_ms not supported on kernel < 5.8");
