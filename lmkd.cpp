...
static int kill_one_process(struct proc* procp, int min_oom_score, struct kill_info *ki,
    ...
    if (!async_kill) {
        // In the case of asynchronous kill, stop_wait_for_proc_kill will be called either from
        // kill_fail_handler if kill failed or from kill_done_handler if kill succeeded.
