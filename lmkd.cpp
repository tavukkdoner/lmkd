...
/* Read lmk property with persist.device_config.lmk_native.<name> overriding ro.lmk.<name> */
#define GET_LMK_PROPERTY(type, name, def) \
    property_get_##type("persist.device_config.lmk_native." name, \
...
static void update_props() {
    ...
    low_ram_device = GET_LMK_PROPERTY(bool, "config.low_ram", false);
    kill_timeout_ms =
        (unsigned long)GET_LMK_PROPERTY(int32, "kill_timeout_ms", 100);
    use_minfree_levels =
        GET_LMK_PROPERTY(bool, "use_minfree_levels", false);
    per_app_memcg =
        GET_LMK_PROPERTY(bool, "config.per_app_memcg", low_ram_device);
