...
void record_low_pressure_levels(union meminfo *mi) {
    int64_t sys_nr_free_page = mi->field.nr_free_pages - mi->field.cma_free;
    if (low_pressure_mem.min_nr_free_pages == -1 ||
        low_pressure_mem.min_nr_free_pages > sys_nr_free_page) {
        if (debug_process_killing) {
    ...
    if (low_pressure_mem.max_nr_free_pages == -1 ||
        (low_pressure_mem.max_nr_free_pages < sys_nr_free_page &&
         sys_nr_free_page - low_pressure_mem.max_nr_free_pages <
