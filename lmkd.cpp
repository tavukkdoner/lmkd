...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
     ...
     */
    if (get_time_diff_ms(&wmark_update_tm, &curr_tm) > 60000) {
        refresh_watermarks = 1;
        has_watermark_boost = watermark_boost_is_enabled();
    }

    if (refresh_watermarks || has_watermark_boost || watermarks.high_wmark == 0) {
        struct zoneinfo zi;
