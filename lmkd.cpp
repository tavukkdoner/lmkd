...
/* direct reclaim state change event handler data */
static struct event_handler_info direct_reclaim_hinfo;
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    if (vs.field.pgscan_direct != init_pgscan_direct) {
        ...
        direct_reclaim_duration_ms =
                (direct_reclaim_start_tm.tv_sec == 0 && direct_reclaim_start_tm.tv_nsec == 0)
                        ? 0
                        : get_time_diff_ms(&direct_reclaim_start_tm, &curr_tm);
    ...
    if (cycle_after_kill && wmark < WMARK_LOW) {
        ...
        check_filecache = true;
    } else if (reclaim == DIRECT_RECLAIM && direct_reclaim_duration_ms != 0 &&
...
static void direct_reclaim_state_change([[maybe_unused]] int data, [[maybe_unused]] uint32_t events,
                                        [[maybe_unused]] struct polling_params* poll_params) {
    struct timespec curr_tm;
    std::vector<mem_event_t> dr_events;
    ...
    if (!memevent_listener->getMemEvents(dr_events)) {
        ALOGE("Failed fetching direct reclaim events.");
        return;
    }
    if (dr_events.empty()) {
        /*
         * Unlike scenario where we are woken up for a state change
         * with no events in the bpf ring buffer
         */
        ALOGE("No direct reclaim events found.");
        return;
    }

    if (dr_events.back().type == MEM_EVENT_DIRECT_RECLAIM_BEGIN) {
        direct_reclaim_start_tm = curr_tm;
    } else if (dr_events.back().type == MEM_EVENT_DIRECT_RECLAIM_END) {
...
static bool init_direct_reclaim_monitoring() {
    ...
    direct_reclaim_hinfo.handler = direct_reclaim_state_change;
    direct_reclaim_hinfo.data = 0;

    if (register_psi_monitor(epollfd, memevent_listener_fd, &direct_reclaim_hinfo) < 0) {
        ALOGE("Failed registering direct reclaim fd: %d", memevent_listener_fd);
        return false;
    }
...
static void destroy_direct_reclaim_monitoring() {
    if (memevent_listener && memevent_listener_fd >= 0) {
        if (unregister_psi_monitor(epollfd, memevent_listener_fd) < 0)
...
static bool init_psi_monitors() {
    ...
    if (use_new_strategy) {
        ...
        /* Use direct reclaim monitoring */
        if (!init_direct_reclaim_monitoring()) destroy_direct_reclaim_monitoring();
    }
