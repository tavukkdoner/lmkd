...
#define LMKD_MONITOR_PARAM_PATH "/sys/module/lmkd_mon/parameters"
#define LMKD_MONITOR_PARAM_PID LMKD_MONITOR_PARAM_PATH "/pid"
#define LMKD_MONITOR_PARAM_ACTIVE LMKD_MONITOR_PARAM_PATH "/active"
...
static int kpoll_fd;
static lmkd_mon_state lmkd_monitor_state = NOT_INITIALIZED;
...
static int kill_one_process(struct proc* procp, int min_oom_score, struct kill_info *ki,
    ...
    /* Stop lmkd monitor watchdog after the kill */
    if (lmkd_monitor_state == ACTIVE)
        writefilestring(LMKD_MONITOR_PARAM_ACTIVE, "0", false);
...
static bool setup_lmkd_monitor() {
    char val[20];

    if (lmkd_monitor_state != NOT_INITIALIZED)
        return true;

    /* System is still booting, so lmkd_mon driver might not be loaded yet */
    if (!property_get_int32("sys.boot_completed", 0))
        return false;

    snprintf(val, sizeof(val), "%d", getpid());
    if (writefilestring(LMKD_MONITOR_PARAM_PID, val, false)) {
        ALOGI("lmkd monitor driver is initialized");
        lmkd_monitor_state = ACTIVE;
    } else {
        ALOGW("lmkd monitor driver is unavailable");
        lmkd_monitor_state = MISSING;
    }
...
static void destroy_monitors() {
    if (use_psi_monitors) {
        ...
        destroy_mp_psi(VMPRESS_LEVEL_MEDIUM);
        destroy_mp_psi(VMPRESS_LEVEL_LOW);
        lmkd_monitor_state = NOT_INITIALIZED;
    } else {
