...
static bool find_victim(int oom_score, int prev_pid, Reaper::target_proc &target) {
    struct proc *procp;
    ...
    target.pid = procp->pid;
    target.pidfd = procp->pidfd;
...
static void watchdog_callback() {
    ...
    for (int oom_score = OOM_SCORE_ADJ_MAX; oom_score >= 0;) {
        ...
        bool killed = target.pidfd < 0 ? kill(target.pid, SIGKILL) == 0 :
                      pidfd_send_signal(target.pidfd, SIGKILL, NULL, 0) == 0;
        if (killed) {
