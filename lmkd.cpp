...
static void apply_proc_prio(struct lmk_procprio params, struct ucred* cred) {
    char path[PROCFS_PATH_MAX];
    char val[20];
    int64_t tgid;
    char buf[pagesize];
...
static void ctrl_command_handler(int dsock_idx) {
    ...
    case LMK_PROCS_PRIO:
        ...
        if (isIoUringSupported) {
            ...
            struct lmk_procs_prio procs_prio;
            lmkd_pack_get_procs_prio(packet, &procs_prio, procs_prio_cnt);
            for (int i = 0; i < procs_prio_cnt; i++) apply_proc_prio(procs_prio.procs[i], &cred);
        }
