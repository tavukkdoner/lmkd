...
/* Read lmk property with persist.device_config.<name> overriding ro.<name> */
#define GET_LMK_PROP(type, name, def) \
    property_get_##type("persist.device_config.lmk." name, \
...
static bool init_psi_monitors() {
    ...
    bool use_new_strategy =
        GET_LMK_PROP(bool, "use_new_strategy", low_ram_device || !use_minfree_levels);
...
static bool init_monitors() {
    ...
    use_psi_monitors = GET_LMK_PROP(bool, "use_psi", true) &&
        init_psi_monitors();
...
static void update_props() {
    ...
    level_oomadj[VMPRESS_LEVEL_LOW] =
        GET_LMK_PROP(int32, "low", OOM_SCORE_ADJ_MAX + 1);
    level_oomadj[VMPRESS_LEVEL_MEDIUM] =
        GET_LMK_PROP(int32, "medium", 800);
    level_oomadj[VMPRESS_LEVEL_CRITICAL] =
        GET_LMK_PROP(int32, "critical", 0);
    debug_process_killing = GET_LMK_PROP(bool, "debug", false);

    /* By default disable upgrade/downgrade logic */
    enable_pressure_upgrade =
        GET_LMK_PROP(bool, "critical_upgrade", false);
    upgrade_pressure =
        (int64_t)GET_LMK_PROP(int32, "upgrade_pressure", 100);
    downgrade_pressure =
        (int64_t)GET_LMK_PROP(int32, "downgrade_pressure", 100);
    kill_heaviest_task =
        GET_LMK_PROP(bool, "kill_heaviest_task", false);
    low_ram_device = GET_LMK_PROP(bool, "config.low_ram", false);
    kill_timeout_ms =
        (unsigned long)GET_LMK_PROP(int32, "kill_timeout_ms", 100);
    use_minfree_levels =
        GET_LMK_PROP(bool, "use_minfree_levels", false);
    per_app_memcg =
        GET_LMK_PROP(bool, "config.per_app_memcg", low_ram_device);
    swap_free_low_percentage = clamp(0, 100, GET_LMK_PROP(int32, "swap_free_low_percentage",
        DEF_LOW_SWAP));
    psi_partial_stall_ms = GET_LMK_PROP(int32, "psi_partial_stall_ms",
        low_ram_device ? DEF_PARTIAL_STALL_LOWRAM : DEF_PARTIAL_STALL);
    psi_complete_stall_ms = GET_LMK_PROP(int32, "psi_complete_stall_ms",
        DEF_COMPLETE_STALL);
    thrashing_limit_pct = max(0, GET_LMK_PROP(int32, "thrashing_limit",
        low_ram_device ? DEF_THRASHING_LOWRAM : DEF_THRASHING));
    thrashing_limit_decay_pct = clamp(0, 100, GET_LMK_PROP(int32, "thrashing_limit_decay",
        low_ram_device ? DEF_THRASHING_DECAY_LOWRAM : DEF_THRASHING_DECAY));
    thrashing_critical_pct = max(0, GET_LMK_PROP(int32, "thrashing_limit_critical",
        thrashing_limit_pct * 2));
    swap_util_max = clamp(0, 100, GET_LMK_PROP(int32, "swap_util_max", 100));
    filecache_min_kb = GET_LMK_PROP(int64, "filecache_min_kb", 0);
}
