...
static void log_pgskip_stats(union vmstat* vs, int64_t* init_pgskip) {
    ...
    for (i = VS_PGSKIP_FIRST_ZONE; i <= VS_PGSKIP_LAST_ZONE; i++) {
        if (vs->arr[i] >= 0) {
            pgskip_deltas[PGSKIP_IDX(i)] = vs->arr[i] - init_pgskip[PGSKIP_IDX(i)];
        }
    }
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    ULMK_LOG(D, "%s pressure event %s", level_name[level], events ? "triggered" : "polling check");
    ...
    if (kill_pending && (kill_timeout_ms == 0 ||
        ...
        wi.skipped_wakeups++;
        ULMK_LOG(D, "Ignoring %s pressure event; kill already in progress", level_name[level]);
        goto no_kill;
    }
    /*
    ...
    if (since_thrashing_reset_ms > THRASHING_RESET_INTERVAL_MS) {
        ...
        ULMK_LOG(D, "thrashing: %" PRId64 "%% thrashing_limit: %d%%", thrashing, thrashing_limit);
    }
    ...
    if (kill_reason != NONE) {
        ...
        if (pages_freed > 0) {
            ...
            ULMK_LOG(D, "No processes to kill with adj score >= %d", min_score_adj);
        }
    } else {
