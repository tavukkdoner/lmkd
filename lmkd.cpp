...
static void cmd_procprio(LMKD_CTRL_PACKET packet, int field_count, struct ucred *cred) {
    ...
    if (params.ptype == PROC_TYPE_APP && per_app_memcg) {
        ...
        snprintf(path, sizeof(path), "%s/uid_%d/pid_%d/%s", memcg_info->apps_dir.c_str(),
                 ...
                 memcg_info->version == 2 ? "memory.low" : "memory.soft_limit_in_bytes");
...
static void killinfo_log(struct proc* procp, int min_oom_score, int rss_kb,
    ...
    android_log_write_int32(ctx, std::min<int32_t>(rss_kb, INT32_MAX));
    android_log_write_int32(ctx, ki ? ki->kill_reason : NONE);

    /* log meminfo fields */
    for (int field_idx = 0; field_idx < MI_FIELD_COUNT; field_idx++) {
        android_log_write_int32(ctx,
                                mi ? std::min<int32_t>(mi->arr[field_idx] * page_k, INT32_MAX) : 0);
    }
    ...
    android_log_write_int32(ctx, std::min<int32_t>(swap_kb, INT32_MAX));
...
static bool __using_memcg_v2() {
    ...
    return memcg_info.has_value() && memcg_info->version == 2;
}

static bool using_memcg_v2() {
    static bool using_v2 = __using_memcg_v2();

    return using_v2;
}

static void run_once(const std::function<void(void)>& f) {
    static bool __attribute__((unused)) done = (f(), true);
};
...
static void mp_event_common(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    // v1 cgroup hierarchy.
    if (using_memcg_v2()) {
        run_once([]() { ALOGE("%s only supports the v1 cgroup hierarchy", __func__); });
        return;
    }
