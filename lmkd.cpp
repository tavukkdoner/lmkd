...
static int zone_name_idx(char *zone_name) {
    static char zone_names[MAX_NR_ZONES][LINE_MAX] =
                          {"DMA", "DMA32", "Normal", "High", "Movable"};
    int i;

    for (i = 0; i < MAX_NR_ZONES; i++) {
        if (strcasecmp(zone_names[i], zone_name) == 0) {
            return i;
        }
    }
...
static int zoneinfo_parse(struct zoneinfo *zi) {
    ...
    for (node_idx = 0; node_idx < zi->node_count; node_idx++) {
        node = &zi->nodes[node_idx];
        ALOGI("node_idx=%d nr_inactive=%" PRId64 " nr_active=%" PRId64, node_idx, node->fields.field.nr_inactive_file, node->fields.field.nr_active_file);
        for (zone_idx = 0; zone_idx < MAX_NR_ZONES; zone_idx++) {
            ...
            ALOGI("zone_idx=%d nr_free=%" PRId64 " nr_cma=%" PRId64 " min=%" PRId64 " low=%" PRId64 " high=%" PRId64,
                    zone_idx, zone->fields.field.nr_free_pages, zone->fields.field.nr_free_cma, zone->fields.field.min, zone->fields.field.low, zone->fields.field.high);
        }
...
void calc_zone_meminfo(struct zoneinfo *zi, struct zone_meminfo *zmi, int64_t *pgskip_deltas) {
    ...
    for (int node_idx = 0; node_idx < zi->node_count; node_idx++) {
        ...
        for (pgskip_idx = VS_PGSKIP_FIRST_ZONE;
             pgskip_idx <= VS_PGSKIP_LAST_ZONE; pgskip_idx++) {
            struct zoneinfo_zone *zone = &node->zones[PGSKIP_IDX(pgskip_idx)];
            int64_t pgskip_delta = pgskip_deltas[PGSKIP_IDX(pgskip_idx)];

            add_zone_watermarks(total_watermarks, zone);

            if (!zone->fields.field.present || pgskip_delta > 0) {
                continue;
            }
...
static bool calc_pgskip_deltas(union vmstat *vs, int64_t *pgskip_deltas)
{
    ...
    ALOGI("MGLRU_status=%s", MGLRU_status ? "true" : "false");
    ...
    for (i = VS_PGSKIP_FIRST_ZONE, pgskip_idx = PGSKIP_IDX(i);
        ...
        */
        ...
        pgskip_changed = pgskip_deltas[pgskip_idx] > 0;
        init_pgskip[pgskip_idx] = vs->arr[i];
        ALOGI("pgskip_idx=%d, init_pgskip=%" PRId64 " pgskip_delta=%" PRId64,
               pgskip_idx, init_pgskip[pgskip_idx], pgskip_deltas[pgskip_idx]);
    }
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
    ...
    ALOGI("zmi: nr_free=%" PRId64 " cma_free=%" PRId64 " min=%" PRId64 " low=%" PRId64 " high=%" PRId64,
                zmi.nr_free_pages, zmi.cma_free, zmi.watermarks.min_wmark, zmi.watermarks.low_wmark, zmi.watermarks.high_wmark);
