...
static int kill_one_process(struct proc* procp, int min_oom_score, int oom_score_actual,
    ...
    result = lmkd_free_memory_before_kill_hook(procp, rss_kb / page_k, oom_score_actual,
...
static int find_and_kill_process(int min_score_adj, struct kill_info *ki, union meminfo *mi,
    ...
    for (i = OOM_SCORE_ADJ_MAX; i >= min_score_adj; i--) {
        ...
        while (true) {
            ...
            killed_size = kill_one_process(procp, min_score_adj, i, ki, mi, wi, tm, pd);
            if (killed_size >= 0) {
