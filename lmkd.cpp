...
static void killinfo_log(struct proc* procp, int min_oom_score, int rss_kb,
    ...
    for (int field_idx = 0; field_idx < MI_FIELD_COUNT; field_idx++) {
        android_log_write_int32(ctx, mi ? std::min<int32_t>(mi->arr[field_idx] * page_k, INT32_MAX): 0);
    }
...
static void update_props() {
    ...
    thrashing_critical_pct = std::max(0, GET_LMK_PROPERTY(int32, "thrashing_limit_critical",
