...
class LmkdTest : public ::testing::Test {
  public:
    ...
    void SendProcsPrioRequest(struct lmk_procs_prio procs_prio_request) {
...
TEST_F(LmkdTest, TargetReaping) {
    GTEST_SKIP() << "For testing purposese we are skipping, remove after debugging";
...
TEST_F(LmkdTest, batch_procs_oom_score_adj) {
    ...
    for (int i = 0; i < PROCS_PRIO_MAX_SIZE; i++) {
        ...
        // clang-format off
        procs_prio_request.procs[i] = {
            .pid = children_info[i].pid,
            .uid = parent_uid,
    ...
    /*
     * Submit batching, then give some time for LMKD to process the request
     * before attempting to read/validate the new OOM scores.
     */
    SendProcsPrioRequest(procs_prio_request);
    sleep(2);
    ...
    for (auto child_info : children_info) {
        ...
        const int actual_new_oom_score = std::stoi(curr_oom_score);
