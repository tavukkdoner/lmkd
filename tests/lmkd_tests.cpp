...
#define LMKD_KILL_MARKER LMKD_LOGCAT_MARKER ": Kill"
#define LMKD_REAP_TIME_MARKER LMKD_LOGCAT_MARKER ": Process"
...
class LmkdTest : public ::testing::Test {
  public:
    ...
    static std::string ReadLogcat() { return ExecCommand("logcat -d -b all"); }

    size_t ConsumeMemory(size_t total_size, size_t step_size, size_t step_delay) {
    ...
    bool ParseProcSize(const std::string& line, long& rss, long& swap) {
    ...
    bool ParseReapTime(const std::string& line, pid_t pid, long& reap_time) {
        int reap_pid;
...
TEST_F(LmkdTest, ReapSpeed) {
    ...
    std::string logcat_out = ReadLogcat();

    // find kill report
    std::string pid_kill_tag = StringPrintf("(%d)", pid);
    size_t kill_pos = std::string::npos;
    size_t line_start;
    size_t line_end = 0;
    long rss, swap;
    std::string line;
    while (true) {
        line_start = logcat_out.find(LMKD_KILL_MARKER, line_end);
        ...
        line = logcat_out.substr(line_start, line_end == std::string::npos ? std::string::npos
                                                                           : line_end - line_start);
        if (line.find(pid_kill_tag) != std::string::npos) {
    ...
    size_t start_pos = kill_pos;
    long reap_time;
    while (true) {
        line_start = logcat_out.find(LMKD_REAP_TIME_MARKER, start_pos);
        ASSERT_TRUE(line_start != std::string::npos) << "Reaping time report is not found";
        line_end = logcat_out.find('\n', line_start);
        line = logcat_out.substr(line_start, line_end == std::string::npos ? std::string::npos
                                                                           : line_end - line_start);
        if (ParseReapTime(line, pid, reap_time)) {
            break;
        }
        // report was not for the right pid, keep searching
        start_pos = line_end;
    }
    ...
    EXPECT_GE(reclaim_speed, RECLAIM_SPEED_MIN);
}
