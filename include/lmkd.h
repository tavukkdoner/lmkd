...
#define PROCS_PRIO_MAX_SIZE ((CTRL_PACKET_MAX_SIZE) / sizeof(struct lmk_procprio))
...
static inline int lmkd_pack_get_procs_prio(LMKD_CTRL_PACKET packet, struct lmk_procs_prio* params,
    ...
    if (field_count < LMK_PROCPRIO_TOTAL_FIELDS || field_count % LMK_PROCPRIO_TOTAL_FIELDS == 0)
        return -1;
    const int procs_count = (field_count / LMK_PROCPRIO_TOTAL_FIELDS);
